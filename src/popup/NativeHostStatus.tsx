...
type Presentation = "connected" | "disconnected";
...
const descriptions: Record<Presentation, string> = {
  connected: "Control Chrome with Codex.",
  disconnected: "Use the Chrome plugin in Codex to connect.",
};

function toPresentation(status: NativeHostStatus | null): Presentation {
  return status?.state === "connected" ? "connected" : "disconnected";
}

export function NativeHostStatusCard({
  showInternalDetails,
}: {
  ...
  useEffect(() => {
    ...
    chrome.storage.local.get(NATIVE_HOST_STATUS_KEY).then((items) => {
      if (mounted) {
        setStatus((items[NATIVE_HOST_STATUS_KEY] as NativeHostStatus) ?? null);
      }
    });
    void refreshStatus();
    ...
    const onChanged = (
      ...
      if (areaName === "local" && changes[NATIVE_HOST_STATUS_KEY]) {
        setStatus(
          (changes[NATIVE_HOST_STATUS_KEY].newValue as NativeHostStatus) ??
            null,
        );
      }
    };
  ...
  const presentation = toPresentation(status);

  return (
    <section className="status-card">
      <div className="status-row">
        <div className={`status-pill status-pill-${presentation}`}>
          <span className="status-dot" aria-hidden="true" />
          <span className="truncate">{labels[presentation]}</span>
        </div>
        <button
          type="button"
          onClick={() => {
            window.location.href = CHROME_SETTINGS_URL;
          }}
      ...
      <p className="status-description">
        ...
        <a href={LEARN_MORE_URL} target="_blank" rel="noreferrer">
          Learn about Codex browser control
        </a>
      </p>
      <div className="status-meta">
        {showInternalDetails ? (
          <p className="truncate">
            Host: {status?.hostName ?? "Not available"}
          </p>
        ) : null}
